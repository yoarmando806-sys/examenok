...
if os.path.exists("precio_gas.jpg"):
    st.image("precio_gas.jpg", caption="Precio estimado de gasolina por estado")
else:
    st.warning("Imagen 'precio_gas.jpg' no encontrada. Continúa sin imagen.")
...
if not os.path.exists('encoder_gasolina.joblib') or not os.path.exists('modelo_gasolina.joblib'):
    ...
    # ----------------------
    try:
        # Convertir solo la columna categórica a string
        ...
        X_encoded = np.hstack([estado_encoded, df[['Mes', 'Año']].values])
